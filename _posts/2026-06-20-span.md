...
즉, 우리가 여기서 이해해야할, 유도해낼 목표는 상한연산이다.
...
라고 정의하면, $\bigvee \mathcal{S} = \bigvee \mathcal{D}$ 이다. 이 식을 기억해두도록 하자.<br>
이 식으로부터, 위의 구조를 얻어내는게 우리의 목표다.
...
두 보조정리를 합해보면, $\mathcal{D}$는 directed set이므로

$$
\begin{align*}
\bigvee \mathcal{S} &= \bigcup \mathcal{D} \\
&= \bigcup \left\{\bigvee \mathcal{F} : \mathcal{F} \subseteq \mathcal{S},\ |\mathcal{F}| < \infty \right\}
\end{align*}
$$
...
우리에게 남은 과제는, 유한한 family의 join에 대해서 이해하는것이고,

이는 곧 binary join에 대한 이해를 의미한다.
